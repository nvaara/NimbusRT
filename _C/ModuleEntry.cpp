...
PYBIND11_MODULE(_C, m)
{
	...
	py::class_<Scene>(m, "NativeScene")
		...
		.def("_sionna_coverage_map", &Scene::ComputeSionnaCoverageMap)
		.def_property_readonly("_size", &Scene::GetSize)
		.def_property_readonly("_center", &Scene::GetCenter);
	...
	py::class_<SionnaCoverageWrapper>(m, "NativeSionnaCoverageMapData")
		...
		.def_property_readonly("targets", &SionnaCoverageWrapper::GetReceivers)
		.def("vertices", &SionnaCoverageWrapper::GetInteractions)
		.def("normals", &SionnaCoverageWrapper::GetNormals)
		.def("objects", &SionnaCoverageWrapper::GetMaterials)
		.def("mask", &SionnaCoverageWrapper::GetMask)
		.def("theta_t", &SionnaCoverageWrapper::GetAodElevation)
		.def("theta_r", &SionnaCoverageWrapper::GetAoaElevation)
		.def("phi_t", &SionnaCoverageWrapper::GetAodAzimuth)
		.def("phi_r", &SionnaCoverageWrapper::GetAoaAzimuth)
		.def("tau", &SionnaCoverageWrapper::GetTimeDelays)
		.def("total_distance", &SionnaCoverageWrapper::GetTotalDistance)
		.def("k_tx", &SionnaCoverageWrapper::GetKTx)
		.def("k_rx", &SionnaCoverageWrapper::GetKRx)
		.def("k_i", &SionnaCoverageWrapper::GetIncidentRays)
		.def("k_r", &SionnaCoverageWrapper::GetDeflectedRays)
		.def("scat_last_objects", &SionnaCoverageWrapper::GetScatLastObjects)
		.def("scat_last_vertices", &SionnaCoverageWrapper::GetScatLastVertices)
		.def("scat_last_k_i", &SionnaCoverageWrapper::GetScatLastIncident)
		.def("scat_k_s", &SionnaCoverageWrapper::GetScatLastDeflected)
		.def("scat_last_normals", &SionnaCoverageWrapper::GetScatLastNormals)
		.def("scat_src_2_last_int_dist", &SionnaCoverageWrapper::GetScatDistToLastIa)
		.def("scat_2_target_dist", &SionnaCoverageWrapper::GetScatDistFromLastIaToRx)
		.def("cos_theta_i", &SionnaCoverageWrapper::GetCosThetaTx)
		.def("cos_theta_m", &SionnaCoverageWrapper::GetCosThetaRx)
		.def("distance_tx_ris", &SionnaCoverageWrapper::GetDistanceTxRis)
		.def("distance_rx_ris", &SionnaCoverageWrapper::GetDistanceRxRis)
		.def("max_link_paths", &SionnaCoverageWrapper::GetMaxLinkPaths)
		.def_property_readonly("shape", &SionnaCoverageWrapper::GetDimensions)
		.def_property_readonly("rx_coords_2d", &SionnaCoverageWrapper::GetRx2D)
		.def_property_readonly("center", &SionnaCoverageWrapper::GetCenter)
		.def_property_readonly("size", &SionnaCoverageWrapper::GetSceneSize)
		.def_property_readonly("cell_size", &SionnaCoverageWrapper::GetCellSize);
