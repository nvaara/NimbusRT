...
namespace Nimbus
{
	...
	public:
		...
		template <typename Type>
		static DeviceBuffer Create(const Type* data, size_t count);
	...
	template <typename Type>
	inline DeviceBuffer DeviceBuffer::Create(const Type* data, size_t count)
	{
		DeviceBuffer result = DeviceBuffer(count * sizeof(Type));
		result.Upload(data, count);
		return result;
	}
